...
class Group(ModelObjectType[models.Group]):
    ...
    @staticmethod
    def resolve_users(root: auth_models.Group, _info: ResolveInfo):
        return root.user_set.all()

    @staticmethod
    def resolve_permissions(root: auth_models.Group, _info: ResolveInfo):
    ...
    @staticmethod
    def resolve_user_can_manage(root: auth_models.Group, info: ResolveInfo) -> bool:
        user = info.context.user
        if not user:
            return False
