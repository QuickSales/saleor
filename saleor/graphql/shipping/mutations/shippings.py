...
class ShippingZoneMixin:
    ...
    def _save_m2m(cls, info: ResolveInfo, instance, cleaned_data):
        with traced_atomic_transaction():
            super()._save_m2m(info, instance, cleaned_data)  # type: ignore[misc]
...
class ShippingMethodTypeMixin:
    ...
    def get_instance(cls, info: ResolveInfo, **data):
        ...
        if object_id:
            instance = cls.get_node_or_error(  # type: ignore[attr-defined]
                info, object_id, qs=models.ShippingMethod.objects
            )
        else:
            instance = cls._meta.model()  # type: ignore[attr-defined]
        return instance
...
class ShippingPriceMixin:
    ...
    def save(cls, info: ResolveInfo, instance, cleaned_input):
        with traced_atomic_transaction():
            super().save(info, instance, cleaned_input)  # type: ignore[misc]
